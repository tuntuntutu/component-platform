...
function getModuleData(props) {
  ...
  let moduleData;
  if (/^(components|docs).*$/.test(pathname)){
    moduleData = [
      ...props.picked.webComponents,
      ...props.picked['docs']
    ];
  }else{
    moduleData = props.picked[pathname]
  }
...
export default class MainContent extends React.Component {
  ...
  generateMenuItem(isTop, item) {
    ...
    const text = isTop ? title : [
      <span key="english" title={title}>{title}</span>,
      <span className="chinese" key="chinese" title={item.subtitle}>{item.subtitle}</span>,
    ];
    const { disabled } = item;
    const url = item.filename.replace(/(\/index)?\.md$/i, '').toLowerCase();
    const child = !item.link ? (
      <Link
        to={utils.getLocalizedPathname(/^components/.test(url) ? `${url}/` : url)}
        disabled={disabled}
      >
        {text}
      </Link>) : (
